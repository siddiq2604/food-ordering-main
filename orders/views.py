...
def checkout(request):
    if request.method == 'POST':
        ...
        if len(list_of_items)!=0:
            order = UserOrder(username=username, order=list_of_items, price=float(price), delivered=False, token=token) #create the row entry
            ...
            ak=""
            for z in list_of_items:
                ak=ak+z+" ,"
            message = "your token number: "+str(token)+"\n Order has been placed successfully"+"\n Your items are "+str(ak)
            from_me = settings.EMAIL_HOST_USER
            to_you  = ["charithagaraga@gmail.com"]
            to_you.append(email)
            print(to_you)
...
def mark_order_as_delivered(request):
    if request.method == 'POST':
        ...
        email=request.POST.get('email')
        subject= "Order details"
        message = "Order has been deliverd"
        from_me = settings.EMAIL_HOST_USER
        to_you  = ["charithagaraga@gmail.com"]
        to_you.append(email)
        print(to_you)
